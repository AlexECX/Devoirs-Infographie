...
function quad(side, top, bottom) {
    ...
    // a^2 + b^2 = c^2
    tri_height = Math.sqrt(s*s - (s/2)*(s/2))
    face( [-s,-s,s, s,-b,s, s,t,s, -s,s,s], [0,0,1] );
    face( [-s,-s,-s, -s,s,-s, s,t,-s, s,-b,-s], [0,0,-1] );
    face( [-s,s,-s, -s,s,s, s,t,s, s,t,-s], [0,1,0] );
    face( [-s,-s,-s, s,-b,-s, s,-b,s, -s,-s,s], [0,-1,0] );
    face( [s,-b,-s, s,t,-s, s,t,s, s,-b,s], [1,0,0] );
...
function triangle_rectangle(side) {
    var s = (side || 1)/2;
    var coords = [];
    var normals = [];
    var texCoords = [];
    ...
    // a^2 + b^2 = c^2
    tri_height = Math.sqrt(s*s - (s/2)*(s/2))
    face( [-s,-s,s, tri_height,-0,s, tri_height,0,s, -s,s,s], [0,0,1] );
    face( [-s,-s,-s, -s,s,-s, tri_height,0,-s, tri_height,-0,-s], [0,0,-1] );
    face( [-s,s,-s, -s,s,s, tri_height,0,s, tri_height,0,-s], [0,1,0] );
    face( [-s,-s,-s, tri_height,-0,-s, tri_height,-0,s, -s,-s,s], [0,-1,0] );
    face( [-s,-s,-s, -s,-s,s, -s,s,s, -s,s,-s], [-1,0,0] );
    return {
       vertexPositions: new Float32Array(coords),
       vertexNormals: new Float32Array(normals),
