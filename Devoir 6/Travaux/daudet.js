...
function createModelbox(modelData) {  // For creating the environment box.
    ...
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, model.indexBuffer);
    gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, modelData.indices, gl.STATIC_DRAW);
    model.render = function (program) {
        gl.bindBuffer(gl.ARRAY_BUFFER, this.coordsBuffer);
        gl.vertexAttribPointer(program.loc("vcoords"), 3, gl.FLOAT, false, 0, 0);
        gl.uniformMatrix4fv(program.loc("modelview"), false, flatten(modelview));
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.indexBuffer);
        gl.drawElements(gl.TRIANGLES, this.count, gl.UNSIGNED_SHORT, 0);
    }
    return model;
...
function createModelmap(modelData) {
    ...
    model.render = function (program, minv) {
        gl.bindBuffer(gl.ARRAY_BUFFER, this.coordsBuffer);
        gl.vertexAttribPointer(program.loc("vcoords"), 3, gl.FLOAT, false, 0, 0);
        gl.bindBuffer(gl.ARRAY_BUFFER, this.normalBuffer);
        gl.vertexAttribPointer(program.loc("vnormal"), 3, gl.FLOAT, false, 0, 0);

        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.indexBuffer);

        gl.uniformMatrix4fv(program.loc("modelview"), false, flatten(modelview));    //--- load flattened modelview matrix
        gl.uniformMatrix3fv(program.loc("normalMatrix"), false, flatten(normalMatrix));  //--- load flattened normal matrix

        //gl.uniformMatrix3fv(program.loc("minv"), false, flatten(minv));  // send matrix inverse of modelview in order to rotate the skybox
